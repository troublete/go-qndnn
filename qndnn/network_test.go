...
func Test_Train(t *testing.T) {
	t.Run("successful", func(t *testing.T) {
		...
		err = nn.Train([]float64{5}, []float64{.666}, learningRate, 1)
		if err != nil {
			t.Error(err)
		}
	...
	t.Run("error-nous run input", func(t *testing.T) {
		nn := NewNetwork(nil, 1, 2, 1)
		err := nn.Train([]float64{1, 2}, []float64{1}, .5, 1)
		if err == nil {
	...
	t.Run("error-nous run output", func(t *testing.T) {
		nn := NewNetwork(nil, 1, 2, 1)
		err := nn.Train([]float64{1}, []float64{1, 2}, .5, 1)
		if err == nil {
			t.Error("expected error got none")
		}
	})
	...
	t.Run("setting negative rounds", func(t *testing.T) {
		nn := NewNetwork(nil, 1, 2, 1)
		err := nn.Train([]float64{1}, []float64{1}, .5, -1)
		if err != nil {
			t.Error(err)
		}
	})
